package com.example.mad105project_sprint1.math
...
class Matrix {
    ...
    var coefficientsAsRationals = arrayOf(
        arrayOf(Rational(0, 1), Rational(0, 1), Rational(0, 1), Rational(0, 1), Rational(0, 1)),
        arrayOf(Rational(0, 1), Rational(0, 1), Rational(0, 1), Rational(0, 1), Rational(0, 1)),
        arrayOf(Rational(0, 1), Rational(0, 1), Rational(0, 1), Rational(0, 1), Rational(0, 1)),
        arrayOf(Rational(0, 1), Rational(0, 1), Rational(0, 1), Rational(0, 1), Rational(0, 1))
    )
    ...
    fun copy(): Matrix {
        ...
        for (row in coefficientsAsRationals.indices) {
            for (column in coefficientsAsRationals[row].indices) {
                temp.coefficientsAsRationals[row][column] =
                    this.coefficientsAsRationals[row][column]
    ...
    fun multiplyRowByConstant(row: Int, constant: String) {
        ...
        if (rational.isRational(constant)) {
            ...
            for (column in coefficientsAsRationals[row - 1].indices) {
                ...
                this.coefficients[row - 1][column] =
                    this.coefficientsAsRationals[row - 1][column].toString()
            }
        }
    }
    ...
    fun rowPlusConstantRow(finalRow: Int, constant: String, pivotRow: Int) {
        ...
        if (rational.isRational(constant)) {
            ...
            for (column in coefficientsAsRationals[finalRow - 1].indices) {
                val temp =
                    coefficientsAsRationals[finalRow - 1][column] + (rational * coefficientsAsRationals[pivotRow - 1][column])
                this.coefficientsAsRationals[finalRow - 1][column] = temp
                this.coefficients[finalRow - 1][column] =
                    this.coefficientsAsRationals[finalRow - 1][column].toString()
            }
        }
    }
}
